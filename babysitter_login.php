...
if (isset($_POST['submit'])) {
	...
	if ($result->num_rows > 0) {
        ...
        $sql = "SELECT * FROM babysitter WHERE `password`='$password'";
        $result = mysqli_query($conn, $sql);
    
        if ($result->num_rows > 0) {
            $sql = "SELECT * FROM babysitter WHERE username='$username' AND password='$password'";
            $result = mysqli_query($conn, $sql);
        
            if ($result->num_rows > 0) {
                $_SESSION['sittername'] = $_POST['username'];

                $sql = "INSERT INTO latest_users(`image`, `name`, `role`) VALUES ((SELECT `sitter_image` FROM babysitter WHERE username='$username'), '$username', 'Babysitter')";
                $result = mysqli_query($conn, $sql);
                if($result){
                    header("Location: babysitter_home.php");
                }
                ...
                <div class="app-auth-body mx-auto">
                    ...
                    <div class="auth-form-container text-start">
                        ...
                        <div class="auth-option text-center pt-5">Don't Have a Account? <a class="text-link"
                                href="babysitter_signup.php">Sign Up Now</a>.</div>
                    </div> 
