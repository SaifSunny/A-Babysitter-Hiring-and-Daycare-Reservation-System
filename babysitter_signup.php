...
if (isset($_SESSION['sittername'])) {
    header("Location: babysitter_home.php");
}
    ...
    if ($password == $cpassword) {
            if (strlen($p) > 5) {
                ...
                if (!$query_run->num_rows > 0) {
                    ...
                    if(!$query_run->num_rows > 0){
                        $query2 = "INSERT INTO babysitter(username,email,`password`,join_date)
                        VALUES ('$username', '$email', '$password', '$date')";
                        $query_run2 = mysqli_query($conn, $query2);

                        if ($query_run2) {
                            $_SESSION['sittername'] = $_POST['username'];
                            echo "<script> alert('Regestration Successfull.');
                            window.location.href='babysitter_profile.php';
                            </script>";
                ...
                <div class="app-auth-body mx-auto">
                    ...
                    <div class="auth-form-container text-start mx-auto">
                        ...
                        <div class="auth-option text-center pt-5">Already have an account? <a class="text-link"
                                href="babysitter_login.php">Log in now.</a></div>
                    </div>
                </div>
                <!--footer-->
