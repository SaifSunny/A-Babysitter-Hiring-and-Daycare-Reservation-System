...
if(isset($_POST['submit'])){
    ...
    if (strlen($p) > 5) {
        ...
        $query = "SELECT * FROM babysitter WHERE username = '$username'";
        $query_run = mysqli_query($conn, $query);
        if (!$query_run->num_rows > 0) {

            $query = "SELECT * FROM babysitter WHERE username = '$username' AND email = '$email'";
            $query_run = mysqli_query($conn, $query);
            if(!$query_run->num_rows > 0){
                ...
                if( in_array($imageFileType,$extensions_arr) ){
                    ...
                    if(move_uploaded_file($_FILES['file']['tmp_name'],$target_dir.$name)){
                        ...
                        $query2 = "INSERT INTO babysitter(username,email,`password`,firstname,lastname,contact,gender,birthday,sitter_image, `address`, city, zip,join_date, verification_status)
                        VALUES ('$username', '$email', '$password', '$firstname', '$lastname', '$contact', '$gender', '$birthday', '$name', '$address', '$city', '$zip','$date','1')";
                        $query_run2 = mysqli_query($conn, $query2);
            
                        if ($query_run2) {
                            $cls="success";
                            $error = "Babysitter Successfully Added.";
                        } 
                        else {
                            $cls="danger";
                ...
                $cls="danger";
                $error = "Babysitter Already Exists";
            }
...
<head>
    <title>Add Babysitter - Kidcare</title>
