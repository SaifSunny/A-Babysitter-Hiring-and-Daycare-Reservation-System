...
$sql = "SELECT * FROM hire_daycare WHERE hire_id='$hire_id'";
...
if(isset($_POST['submit'])){
    ...
    if(!$query_run->num_rows > 0){
        $query2 = "INSERT INTO daycare_ratings(hire_id,`user_id`, daycare_id, user_img, daycare_img, `user_name`, daycare_name, `rating`, `comment`)
        VALUES ('$hire_id','$user_id', '$daycare_id', '$user_img', '$daycare_img', '$user_name', '$daycare_name','$rating','$comment')";
        $query_run2 = mysqli_query($conn, $query2);
                
        if ($query_run2) {
            echo "<script> 
            alert('Rating Successfull');
            window.location.href='user_daycare_appointments.php';
            </script>";
        } 
        else {
          $cls="danger";
        ...
        $query2 = "UPDATE daycare_ratings SET rating = '$rating', comment='$comment' WHERE hire_id='$hire_id'";
        $query_run2 = mysqli_query($conn, $query2);
...
<head>
    <title>Rate Daycare - KidCare</title>
                            ...
                            <div class="row" style="padding-top:30px;">
                                <div class="col-md-12">
                                    <div class="card mx-auto" style="padding:50px 0px; padding-left:40px;">
                                        <h5 class="card-title" style="padding-left:50px;">Rate daycare</h5>
                                        <hr>
                                        <div class="card-body" style="padding:0 60px;">
                                            <form action="" method="POST">
                                                ...
                                                <label for="rating" style="padding-bottom:20px;">Rate the daycare</label>
                                                ...
                                                <div class="text-end"><button class=" btn app-btn-primary"
                                                        style="margin-top:20px;margin-right:60px;" type="submit" name="submit">Rate daycare</button></div>
                                            </form>
                                        </div>
                                    </div>
